...
from src.metrics.srpc_metrics_types import SrpcmetricTypes
import logging
...
class SrpcMetric(SrpcMetricsInterface):
    ...
    def inc_counter_sucess(self, metric_name):
        if f"{metric_name}.{SrpcmetricTypes.COUNTER_SUCESS}" in self.__mestrics:
            self.__logger.info(f"{metric_name}.{SrpcmetricTypes.COUNTER_SUCESS}=1")

    def inc_counter_fail(self, metric_name):
        if f"{metric_name}.{SrpcmetricTypes.COUNTER_FAIL}" in self.__mestrics:
            self.__logger.info(f"{metric_name}.{SrpcmetricTypes.COUNTER_FAIL}=1")
    
    def record_time(self, metric_name, time_taken):
        if f"{metric_name}.time" in self.__mestrics:
            self.__logger.info(f"{metric_name}.{SrpcmetricTypes.TIME}={round(time_taken * 1000, 3)}")