...
def get_interface_from_module(module: ModuleType) -> ABC:
    ...
    try:
        if not issubclass(interface, ABC):
            raise TypeError(f"Module {module.__name__}, does not contain an class that implement ABC class(an interface).")
        if not interface_name == expected_interface_name:
            raise NameError(f"Module {module.__name__}, does not contain an interface class called {expected_interface_name}.")
...
def get_methodname_signature_map_from_interface(interface) -> dict[str, str]:
    ...
    try:
        ...
        logger.info(f"Methods and signatures extracted successfully from interface {interface.__name__}.")
        return dictionary
